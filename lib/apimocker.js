...
apiMocker.createServer = function(options) {
	apiMocker.options = options = _.defaults(options || {}, apiMocker.defaults);
	...
	if (!!options.uploadRoot) {
		apiMocker.middlewares.push(multer({ storage: multer.diskStorage({
			destination: untildify(options.uploadRoot),
			filename: options.useUploadFieldname ? 
				function(req, filename, cb) { cb(null, filename.fieldname); } :
				function(req, filename, cb) { cb(null, filename.originalname); }
		}) }).any());
	}
...
apiMocker.fillTemplate = function(data, req){
	...
	for(var templateString in req.params){
		...
		// Search for '"@@key"' in JSON template, replace with value (no double quotes around final value)
		data = data.replace(new RegExp('"@@'+templateString+'"','g'), req.params[templateString]);
	...
	setTimeout(function() {
		...
		if (options.switch && options.jsonPathSwitchResponse) {
			...
			try {
				...
				var allElems = jp.query(JSON.parse(mockFile), jpath);
		...
		fs.exists(mockPath, function(exists) {
			if (exists) {
				if (options.contentType) {
					...
					res.status(options.httpStatus || 200).sendFile(options.mockFile, {root: apiMocker.options.mockDirectory});
				}
			} else {
				res.sendStatus(options.httpStatus || 404);
			}
	...
	switches.forEach(function(s){
		...
		if (!switchParamValue) {
			...
			if (switchObject.type === 'regexp') {
				...
				var allElems = jp.query(req.body, switchObject.switch);	// jshint ignore:line
