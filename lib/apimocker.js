...
apiMocker.getServiceRoute = function(path, verb) {
  ...
  if (finalSvc) {
    finalSvc.verb = !_.isEmpty(verb) ? verb.toLowerCase() : '';
...
apiMocker.sendResponse = function(req, res, serviceKeys) {
  ...
  if (options) {
    ...
    apiMocker.log("ERROR: No mock web service was configured for route: " + serviceKeys.serviceUrl);
      res.status(404).send({
        apimockerError: 'No mock web service was configured for route.  Check apimocker config.json file.',
        route: serviceKeys.serviceUrl
      });
    }
};
