...
apiMocker.getFiles = function(dir, files_){
    files_ = files_ || [];
    try {
...
apiMocker.sendResponse = function(req, res, serviceKeys) {
  ...
  setTimeout(function() {
    ...
    if (!options.mockFile && !options.genericParams) {
    ...
    if(options.genericParams){
      ...
      params.forEach(function(part, index, array) {
        array[index] = isFinite(part) ? "param_number_"+(index+1) : part;
      });

      var filesInDir = apiMocker.getFiles(path.join(apiMocker.options.mockDirectory, path.normalize(params.join("/"))))
      var file = filesInDir[Math.floor(Math.random()*filesInDir.length)];

      options.mockFile = path.join(path.normalize(params.join("/")),file);
    }
