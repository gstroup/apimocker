...
apiMocker.loadConfigFile = function () {
    if (apiMocker.configFilePath) {
        ...
        var newOptions = _.clone(apiMocker.defaults), configJson;

        if (path.extname(apiMocker.configFilePath) === '.js') {
            var exportedValue = require(apiMocker.configFilePath);
            if (typeof exportedValue === 'function') {
                configJson = exportedValue();
            }
            else {
                configJson = exportedValue;
            }
...
apiMocker.setRoute = function (options) {
    ...
    apiMocker.log('Set route: ' + options.verb.toUpperCase() + ' ' + chalk.blue(`http://localhost:${apiMocker.options.port}/${options.serviceUrl}`) + ' : ' +
        chalk.green(displayFile) + ' ' + displayLatency);
    if (options.switch) {
...
apiMocker.start = function (port, callback) {
    apiMocker.options.port = port || apiMocker.options.port;
    ...
    apiMocker.expressInstance = apiMocker.express.listen(apiMocker.options.port, callback);
    apiMocker.log('Mock server listening on port ' + apiMocker.options.port);
    return apiMocker;
};
